...
const InputOverlay = ({ onSubmit, loading, initialTopic, initialMode, isEditing, onCancel }) => {
    ...
    // When editing a different map, sync the pre-filled values
    useEffect(() => {
        if (isEditing) {
            setValue(initialTopic || '');
            setSelectedMode(initialMode || DEFAULT_MODE);
        }
    }, [isEditing, initialTopic, initialMode]);

    // Rotate suggestions when not editing
    useEffect(() => {
        if (isEditing) return;
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (canSubmit) onSubmit(value.trim() || 'My Data', selectedMode);
    };
    ...
    return (
        <div style={{
            ...
            gap: '28px', pointerEvents: 'none', zIndex: 100
        }}>
            {/* Hero Title */}
            <div style={{ pointerEvents: 'none', textAlign: 'center', animation: 'fadeInUp 0.8s ease-out' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginBottom: '16px' }}>
                    <NeuroAvatar state={isEditing ? 'happy' : 'idle'} size={28} />
                    <h1 style={{
                        ...
                        {isEditing ? 'Edit Map' : 'Neuro'}
                    </h1>
                </div>
                <p style={{ fontFamily: 'var(--font-body)', fontSize: '15px', color: 'var(--text-secondary)', fontWeight: '400', letterSpacing: '0.2px' }}>
                    {isEditing ? 'Update the topic or switch to a different mode' : 'Turn any topic into a visual knowledge universe'}
                </p>
            </div>

            {/* Mode Selector */}
            <div style={{
                pointerEvents: 'auto', display: 'flex', gap: '8px', flexWrap: 'wrap',
            ...
            {/* Mode Description */}
            {activeMode && (
                <p style={{ pointerEvents: 'none', fontFamily: 'var(--font-body)', fontSize: '12px', color: 'var(--muted)', textAlign: 'center', animation: 'fadeInUp 0.4s ease-out', marginTop: '-16px' }}>
            ...
            <form
                ...
                style={{
                    ...
                    borderRadius: '16px', padding: '6px 8px 6px 12px',
                    display: 'flex', alignItems: 'center', gap: '8px',
                    boxShadow: isListening
                        ? '0 0 0 2px rgba(0,212,255,0.2), var(--shadow-lg)'
                        : 'var(--shadow-lg), 0 0 0 1px rgba(0,212,255,0.05), inset 0 1px 0 rgba(255,255,255,0.04)',
            ...
            >
                ...
                <textarea
                    ...
                    placeholder={
                        ...
                        isEditing ? 'Update the map topic...' :
                    ...
                    style={{
                        ...
                        fontWeight: '400', width: '100%', padding: '12px 0',
                        resize: 'none', maxHeight: '35vh', overflowY: 'auto',
                ...
                {/* Mic button */}
                {isSTTSupported && !isDataMode && (
                    <button
                        type="button"
                        onClick={handleMic}
                        disabled={loading}
                        title={isListening ? 'Stop recording' : 'Speak your topic'}
                        style={{
                            flexShrink: 0, background: isListening ? 'rgba(0,212,255,0.15)' : 'transparent',
                            border: `1px solid ${isListening ? 'var(--accent-cyan)' : 'transparent'}`,
